...
async def anomaly_scheduler():
    ...
    while True:
        try:
            ...
            print("Anomaly scheduler error:", e)
...
async def startup():
    ...
    # 🔥 start anomaly background task
    asyncio.create_task(anomaly_scheduler())
...
async def anomaly_scan():
    ...
    try:
        ...
        # Broadcast critical/warning anomalies to dashboard
        for r in results:
...
async def patchcore_setup(
    ...
    Supports any product type on any factory floor.
    """
    ...
    # Notify dashboard that new product was onboarded
    await manager.broadcast({
...
async def patchcore_status():
    ...
    Shows which product is loaded and memory bank size.
    """
...
async def patchcore_inspect(
    ...
    No training needed — compares against reference
    images uploaded during product setup.
    Works for any product type immediately.
    """
